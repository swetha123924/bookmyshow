...
//   const numericPrice = Number(movie.price); // Ensure number
...
//     price += Number(getSeatPrice(row)); // 🧠 ensure each price is a number
//   });
//   setTotalPrice(price); // 🧼 number always
// }, [selectedSeats]);
...
//         {Array.from({ length: totalColumns }, (_, i) => i + 1).map((seatNumber) => {
//           const seatId = `${rowLabel}${seatNumber}`;
...
//   // if (!user_id || !show_id || !movie?.id) return alert("Missing required booking details");
